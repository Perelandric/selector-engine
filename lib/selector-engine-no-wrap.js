...
function prevElemSib(el) {
  if (LEGACY) {
    while ((el = el.previousSibling) && el.nodeType !== Node.ELEMENT_NODE){}
...
function nextElemSib(el) {
  if (LEGACY) {
    while ((el = el.nextSibling) && el.nodeType !== Node.ELEMENT_NODE){}
...
function SelectorGroup(strLex) {
  ...
  this.isGlobalQualifier = this.globalQualifier === "*"
}
...
SelectorGroup.prototype.selectFirstFrom = function(root) {
  ...
  for (let i = 0, len = p.length; i < len; i+=1) {
    ...
    // If not an element, or an element but not a match, try the next elem
    if (this.matches(root, p[i], this.isGlobalQualifier)) {
      return p[i]
    }
  }
