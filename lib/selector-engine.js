...
var PSEUDO_ELEMENT_REUSE = new Token(PSEUDO_TOKEN);
...
function fieldMatch(target, pttrn) {
  var idx = 0;
  while ((idx = target.indexOf(pttrn, idx)) !== -1) {
    if (getSpaceAt(target, idx - 1) && getSpaceAt(target, idx + pttrn.length)) {
      return true;
    }
    idx = idx + pttrn.length + 1;
  }
  return false;
}
