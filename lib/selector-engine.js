...
Lexer.prototype.getPseudoFunction = function(name) {
  switch(name.toLowerCase()) {
    case "not":
      ...
      return new Token(PSEUDO_FUNCTION_TOKEN, new SelectorGroup(new Lexer(this, ")", true, false)), NOT_TOKEN);
    case "matches":
...
function SelectorGroup(strTok) {
  ...
  while (n = source.nextAfterSpace()) {
    ...
    if (this.qualifiedNames) {
      var q = selObject.qualifier;
      if (q === "*") {
        this.qualifiedNames = null;
      } else {
        if (!contains(this.qualifiedNames, q)) {
...
SelectorGroup.prototype.getPotentials = function(root) {
  var qn = this.qualifiedNames;
  if (qn && qn.length === 1) {
    return root.getElementsByTagName(qn[0]);
  }
  var p = root.getElementsByTagName("*");
  var plen = p.length;
  if (!qn || !plen) {
    return p;
  }
